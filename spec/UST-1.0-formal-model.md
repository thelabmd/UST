...
`t`). What the anchor journal makes adapted is the **commitment process** `Hₜ := contentHash(Rₜ)`: `σ(Hₜ) ⊆ Fₜ`,
witnessed by `Hₜ(ω) ∈ Lₜ(ω)` whenever `Rₜ(ω) ≠ ⊥`. The FULL document `Rₜ` is NOT `Fₜ`-measurable — a hash does
not reveal its preimage (this is exactly UST's design: the anchor fixes that the bytes EXISTED, it neither
stores nor discloses them; §11.2, [[principle_hash_bridges_open_protocol_paid_data]]). `Rₜ` is adapted only to
the richer **base world filtration** `𝒢ₜ := Fₜ ∨ σ( Rᵤ : u ≤ t )` — the join of the journal with the published
records up to `t`; `σ(Rₜ) ⊆ 𝒢ₜ` holds by construction, and `𝒢ₛ ⊆ 𝒢ₜ` because both components accumulate
monotonically. `prev` binds `Rₜ` to `Rₜ₋₁` inside this filtration. **Completeness over `[t₀, t₁]`** is a RANGE property (a
...
With `S_ABS` the vector of absolute STRUCTURE bounds:
  ...
  `INDETERMINATE(unavailable)` if `κ₀ ⋡ μ_V(R) ⪯ κ_ABS` and no trusted grant is in `ℐ_v` — evidence missing;
...
exactly when `sup { C_e(R) : e ∈ 𝓔₀, μ_V(R) ⪯ κ₀, μ_S(R) ⪯ S_ABS } ⪯ inf_{e∈𝓔₀} ρ_e` (the sup ranges over BOTH
environments and floor-sized documents — the cost index `e` is bound by it, not free) — every transcript inside the floor is both
