...
π_override projection applied explicitly BEFORE assembly). **K3:** `deriveAssurance` now takes ONLY a branded `PredicateGraph` handle (minted by `provePredicates` over seam verdicts); a caller-shaped `{identity:'authoritative'}` is not a graph, so no coordinate lifts — round-3 P0-4 closed at the type level (*"K3 deriveAssurance REJECTS a caller-shaped object (not a PredicateGraph) → E-ASSURANCE (round-3 P0-4 closed)"*).
