...
- **`Lₜ : Ω → LogSpace`** — the anchor journal's committed-hash SET at real time `t` in history `ω`. Append-only
  is a property of the coordinate itself: along every path, `s ≤ t ⇒ Lₛ(ω) ⊆ Lₜ(ω)` (a substrate entry exists in
  `Lₜ` only at its registered finality, §11.2 — a rollbackable confirmation is not yet in `LogSpace`).
...
Let the external append-only log (the anchor substrate, §11) define, for each real time `t`:
  ...
  **Fₜ := σ( { E_h : h ∈ HashSpace } ),**  where **`E_h := { ω ∈ Ω : h ∈ Lₜ(ω) }`** — the event "in history
  `ω`, the journal coordinate `Lₜ` (F.1) contains `h` by real time `t`". The σ-algebra is generated by these
  SUBSETS OF Ω; the hash string is only the event's index, and "the log" is not a free-floating notion but the
  explicit coordinate `Lₜ` declared in F.1 — no self-reference remains.

**Proposition F.3 (Fₜ is a filtration).** `{Fₜ}` is a filtration: each `Fₜ` is a σ-algebra, and `s ≤ t ⇒ Fₛ ⊆ Fₜ`.

*Proof.* Each `Fₜ` is generated by a family of subsets of Ω, hence a σ-algebra. By the append-only property of
the COORDINATE (F.1: `Lₛ(ω) ⊆ Lₜ(ω)` for `s ≤ t`, along every path), every generator `E_h` of `Fₛ` is a
generator of `Fₜ`; the generated σ-algebras therefore nest, `Fₛ ⊆ Fₜ`. ∎
...
`t` in this history). The stream is **adapted** in the precise sense `σ(Rₜ) ⊆ Fₜ`, witnessed constructively:
`contentHash(Rₜ(ω)) ∈ Lₜ(ω)` whenever `Rₜ(ω) ≠ ⊥` — the frame's hash is in the journal coordinate by its anchor
time, and `prev` binds `Rₜ` to `Rₜ₋₁`, so the chain is measurable with respect to the growing filtration. **Completeness over `[t₀, t₁]`** is a RANGE property (a
