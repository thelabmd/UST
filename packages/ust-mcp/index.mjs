...
export const tools = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', required: ['doc', 'genesis', 'keylog'], properties: { doc: { type: 'object' }, genesis: { type: 'object' }, keylog: { type: 'array' }, noForkConfirmed: { type: 'boolean' }, anchorTime: { type: 'string' }, keylogFreshAsOf: { type: 'string', description: '#40 RFC3339-Z of an authoritative key-log fetch → freshness:fresh' }, keylogHeadAnchor: { type: 'object', description: '#40 a verified anchor inclusion proof for the key-log head → freshness:attested (a raw hash is not accepted, rc.28)' } } },
    handler: ({ doc, genesis, keylog, noForkConfirmed, anchorTime, keylogFreshAsOf, keylogHeadAnchor }) => P.resolveAuthority(doc, { genesis, keylog, noForkConfirmed, anchorTime, keylogFreshAsOf, keylogHeadAnchor }),
  },
  {
