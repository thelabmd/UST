...
// ─── P1-04 — checkpoint-authority root RESOLVED from the signed genesis, not a raw caller pin ───────────────────
{
  ...
  check('K4 verifyAuthorityBundle (raw genesis + checkpoint + receipts) → VALID corroborated, scope_id + Horn derivation trace', (r => r.result === 'VALID' && r.keylog_freshness === 'corroborated' && r.scope_id === P.authorityScopeId(P.contentHash(genCA)) && r.derivation.some((t) => t.rule === 'FreshnessCorroborated') && r.proven_atoms.includes('checkpoint-after-target'))(P.verifyAuthorityBundle(bundleIn, cfg)));
  ...
  check('K4 bundle with policy.allowExperimentalAttested → attested', (r => r.keylog_freshness === 'attested' && r.proven_atoms.includes('checkpoint-unique') && r.derivation.some((t) => t.rule === 'FreshnessAttested'))(P.verifyAuthorityBundle({ ...bundleIn, uniqueness: uniqK }, { trust: { connectors: connK }, policy: { allowExperimentalAttested: true } })));
